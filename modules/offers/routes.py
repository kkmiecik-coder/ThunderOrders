...
def _build_bonuses_config(page, sections):
    ...
    for section in sections:
        ...
        ).order_by(OfferSetBonus.sort_order).all()
...
def order_page(token):
    """
    Główna strona ofertowa - wyświetla odpowiedni widok w zależności od statusu
    ...
    if page.is_draft:
        return render_template('offers/draft.html', page=page)
    ...
    if page.is_ended:
        return render_template('offers/ended.html', page=page)

    # Fallback
    abort(404)


@offers_bp.route('/<token>/thank-you')
def thank_you(token):
    ...
    return render_template('offers/thank_you.html', page=page, order_data=order_data)


@offers_bp.route('/<token>/preview')
@login_required
def preview_page(token):
    """
    Podgląd strony ofertowej dla admina/moda
...
@offers_bp.route('/<token>/reserve', methods=['POST'])
@csrf.exempt
...
def reserve(token):
    ...
    from .reservation import reserve_product
    ...
    if success:
        try:
            from .socket_events import emit_reservations_update, broadcast_availability_update
...
@offers_bp.route('/<token>/release', methods=['POST'])
@csrf.exempt
@limiter.limit("120 per minute")
def release(token):
    """Release product"""
...
@offers_bp.route('/<token>/availability', methods=['GET'])
@limiter.limit("120 per minute")
def availability(token):
    """Get availability snapshot"""
    from .reservation import get_availability_snapshot
    from .models import OfferSection, OfferSetItem
    from modules.products.models import Product, VariantGroup, variant_products
...
@offers_bp.route('/<token>/place-order', methods=['POST'])
@csrf.exempt
...
def subscribe_notification(token):
    ...
    # Sprawdź czy produkt jest na tej stronie ofertowej
    product_on_page = False

    # Sprawdź sekcje typu 'product'
    product_section = OfferSection.query.filter_by(
        offer_page_id=page.id,
        section_type='product',
        product_id=product_id
    ).first()
