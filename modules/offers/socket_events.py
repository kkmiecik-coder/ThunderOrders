"""
Offers Module - SocketIO Event Handlers
=======================================

Real-time WebSocket events dla stron Offer:
...
def _get_visitor_room(page_id, page_type):
    """Zwraca nazwę rooma dla odwiedzających stronę offer."""
    return f'offer_page_{page_id}_{page_type}'


def _get_admin_room(page_id):
    """Zwraca nazwę rooma dla admin LIVE dashboard."""
    return f'offer_admin_{page_id}'
...
def _get_visitor_counts(page_id):
    ...
    try:
        ...
        counts['active_reservations'] = OfferReservation.query.filter_by(
...
def _check_notification_subscriptions(page_id, current_availability):
    ...
    try:
        from .models import OfferProductNotificationSubscription
        from modules.products.models import Product

        subscriptions = OfferProductNotificationSubscription.query.filter(
            OfferProductNotificationSubscription.offer_page_id == page_id,
            OfferProductNotificationSubscription.product_id.in_(newly_available),
            OfferProductNotificationSubscription.notified == False
        ).all()
            ...
            if user_sid and user_sid in _connected_clients:
                ...
                try:
                    ...
                    if email:
                        ...
                        page_url = ''
                        try:
                            page_url = url_for('offers.order_page',
...
def _schedule_expiry_timer(page_id):
    ...
    with _expiry_lock:
        ...
        now = int(time.time())
        earliest = OfferReservation.query.filter(
            OfferReservation.offer_page_id == page_id,
            OfferReservation.expires_at > now
        ).order_by(OfferReservation.expires_at.asc()).first()
...
def handle_join_offer(data):
    """
    Klient (odwiedzający) dołącza do rooma strony offer.
...
@socketio.on('join_offer_admin')
def handle_join_offer_admin(data):
    """
...
@socketio.on('join_offer_reservation')
def handle_join_offer_reservation(data):
    """
    Klient dołącza do systemu rezerwacji offer (z deduplikacją).
    ...
    try:
        ...
        if transferred_from_session:
            try:
                ...
                old_reservations = OfferReservation.query.filter(
                    OfferReservation.session_id == transferred_from_session,
                    OfferReservation.offer_page_id == page_id,
                    OfferReservation.expires_at > now
                ).with_for_update().all()
                ...
                if old_reservations:
                    for res in old_reservations:
                        ...
                        existing = OfferReservation.query.filter_by(
        ...
        import traceback
        print(f"[SOCKET] join_offer_reservation ERROR: {e}")
        traceback.print_exc()
...
def handle_release_product(data):
    ...
    Data: { page_id, session_id, product_id, quantity }
    """
    from .reservation import release_product
...
def handle_extend_reservation(data):
    ...
    """
    from .reservation import extend_reservation
