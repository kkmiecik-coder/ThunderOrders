...
INSERT INTO feedback_questions (survey_id, question_type, content, options, is_required, sort_order, created_at) VALUES
(@survey_id, 'checkbox_list', 'Zadania do wykonania - Exclusive Orders', '["Otwórz link exclusive i poczekaj na countdown", "Po odblokowaniu, przejrzyj dostępne produkty", "Dodaj co najmniej 2 produkty do koszyka z różnymi ilościami", "Zmień ilość jednego produktu w koszyku", "Usuń jeden produkt z koszyka", "Wypełnij dane do wysyłki (lub wybierz zapisany adres)", "Złóż zamówienie i sprawdź potwierdzenie"]', 1, 2, NOW());
...
INSERT INTO feedback_questions (survey_id, question_type, content, options, is_required, sort_order, created_at) VALUES
(@survey_id, 'checkbox_list', 'Zadania do wykonania - Panel Klienta', '["Zaloguj się do panelu klienta", "Przejrzyj dashboard - sprawdź statystyki i ostatnie zamówienia", "Przejdź do historii zamówień i zobacz szczegóły jednego zamówienia", "Dodaj nowy adres wysyłkowy", "Edytuj istniejący adres wysyłkowy", "Ustaw adres jako domyślny", "Dodaj nową metodę płatności", "Przejrzyj ustawienia konta", "Zmień motyw (light/dark mode)", "Wyloguj się i zaloguj ponownie"]', 1, 28, NOW());
...
INSERT INTO feedback_questions (survey_id, question_type, content, options, is_required, sort_order, created_at) VALUES
(@survey_id, 'multiple_choice', 'Czy poleciłbyś ten system innym?', '["Tak, zdecydowanie", "Raczej tak", "Może", "Raczej nie", "Nie"]', 1, 54, NOW());

INSERT INTO feedback_questions (survey_id, question_type, content, options, is_required, sort_order, created_at) VALUES
(@survey_id, 'textarea', 'Dodatkowe uwagi i sugestie (opcjonalne)', NULL, 0, 55, NOW());

-- PODSUMOWANIE
SELECT s.id AS survey_id, s.name AS survey_name, s.token AS survey_token, s.status, COUNT(q.id) AS questions_count
FROM feedback_surveys s
LEFT JOIN feedback_questions q ON q.survey_id = s.id
WHERE s.id = @survey_id
GROUP BY s.id;

-- Link do ankiety: https://thunderorders.cloud/feedback/test-exclusive-panel-2025
