...
function initializeLiveTabs() {
    ...
    tabButtons.forEach(function (button) {
        button.addEventListener('click', function () {
            ...
            try {
                localStorage.setItem('exclusiveLiveActiveTab', targetTab);
            } catch (e) { /* ignore */ }
        });
    });

    try {
        var savedTab = localStorage.getItem('exclusiveLiveActiveTab');
        if (savedTab) {
...
function initializeSocketIO() {
    ...
    var pageId = window.EXCLUSIVE_PAGE_ID;
    if (!pageId) return;
    ...
    socket.on('connect', function () {
        ...
        socket.emit('join_exclusive_admin', { page_id: pageId });
    });
