/**
 * Payment Upload Page - Mobilna strona uploadu potwierdzenia platnosci (QR code flow)
 * Obsluguje wybor pliku (JPG, PNG, PDF), podglad, wysylanie na serwer
 */
...
(function () {
    ...
    var previewPdfName = document.getElementById('preview-pdf-name');
    ...
    uploadInput.addEventListener('change', function () {
        ...
        if (!file.type.startsWith('image/') && file.type !== 'application/pdf') {
            showError('Wybierz plik graficzny lub PDF (JPG, PNG, PDF)');
            uploadInput.value = '';
            return;
        }
        ...
        if (file.size > 5 * 1024 * 1024) {
            showError('Plik jest za duzy. Maksymalny rozmiar to 5MB.');
            uploadInput.value = '';
            return;
        }

        if (file.type === 'application/pdf') {
            // PDF - pokaz ikonke z nazwa pliku zamiast podgladu
            ...
            // Obraz - odczyt i wyswietlenie podgladu
    ...
    if (removePreview) {
        removePreview.addEventListener('click', function () {
            ...
            if (previewPdf) previewPdf.style.display = 'none';
            if (previewPdfName) previewPdfName.textContent = '';
    ...
    uploadBtn.addEventListener('click', function (e) {
        ...
        var file = uploadInput.files[0];
        if (!file) {
            showError('Najpierw wybierz plik');
            return;
        }
    ...
    function showSuccessState() {
        ...
        card.innerHTML =
            '<div class="upload-icon upload-icon--success">' +
                ...
                '</svg>' +
            '</div>' +
            '<h1 class="upload-title">Potwierdzenie przeslane!</h1>' +
    ...
    function resetUploadBtn() {
        uploadBtn.disabled = false;
        uploadBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg> Wyslij potwierdzenie';
